...
Description:
    Analyze rE2G-predicted enhancer-gene paris with and without cohesin 
...
Inputs:
    - A CSV consisting of thresholded rE2G predictions in the presence of cohesin (noAux, in this case equivalent to Supplememtary Table 1a)
    - A CSV consisting of thresholded rE2G predictions in the absence of cohesin (Aux, in this case equivalent to Supplememtary Table 1b)
...
Outputs:
    - Rolling average plots of Hi-C contact, rE2G score, and enhancer retention (and scatter plots of raw data)
    - Boxplots and barplots comparing enhancer categories across expression groups
    - Per-gene rE2G enhancer summary tables
...
# ------------------ HELPER FUNCTIONS ------------------
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze rE2G enhancer-gene predictions with/without cohesin and stratify by CRUDO expression")
    parser.add_argument("--predictions_noAux", help="Thresholded rE2G predictions in the presence of cohesin (ie SupplementaryTable 1a)")
    parser.add_argument("--predictions_Aux", help="Thresholded rE2G predictions in the absence of cohesin (ie SupplementaryTable 1b)")
...
def add_relevant_columns(df):
    ...
    #Add columns for HiC and rE2G changes
    # Hi-C changes
    ...
    # E2G changes
    rE2G_FC = df['ENCODE-rE2G.Score.Aux'] / df['ENCODE-rE2G.Score.noAux']
    df['ENCODE-rE2G.Score.log2FC.(Aux/noAux)'] = np.log2(df['ENCODE-rE2G.Score.Aux']) - np.log2(df['ENCODE-rE2G.Score.noAux'])
    df['%Change.ENCODE-rE2G.Score'] = (rE2G_FC - 1)
...
def compute_rolling_metrics(df, sort_by='DistanceToTSS.Kb'):
    ...
    col_settings = {
        ...
        'ENCODE-rE2G.Score.noAux': (1000, 100),
        'ENCODE-rE2G.Score.Aux': (1000, 100),
        'ENCODE-rE2G.Score.log2FC.(Aux/noAux)': (1000, 100),
...
def plot_rE2G_score_rolling(df, output_dir, filename=None):
# plots rE2G score (cohesin present and absent) as a rolling circle average (like Fig. 1c)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:   
        save_path = None
    fig, ax = plot_rolling_with_ci(
        df,
        x_col="DistanceToTSS.Kb",
        y_noAux_col="Rolling_ENCODE-rE2G.Score.noAux",
        y_Aux_col="Rolling_ENCODE-rE2G.Score.Aux",
        ci_noAux_col="Rolling_ENCODE-rE2G.Score.noAux.ci95",
        ci_aux_col="Rolling_ENCODE-rE2G.Score.Aux.ci95",
        rasterize=True,
        save_path=save_path
         )
...
def plot_log2fc_rE2G_score_rolling(df, output_dir, filename=None):
    ...
    fig, ax= plot_single_rolling_with_ci(
        data=df,
        x_col="DistanceToTSS.Kb",
        y_col="Rolling_ENCODE-rE2G.Score.log2FC.(Aux/noAux)",
        ci_col="Rolling_ENCODE-rE2G.Score.log2FC.(Aux/noAux).ci95",
        color='k',
        rasterize=True,
        #ylim=(-2.5,0),
        save_path=save_path
...
def plot_rE2G_score_scatter(df, output_dir, filename=None):
    # plots rE2G score scatter (cohesin present and absent) dual plot (like Extended Data Fig. 1i)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:
        save_path = None
    fig, axes = plot_rasterized_scatter_dual(
        df,
        x_col="DistanceToTSS.Kb",
        y_noAux_col="ENCODE-rE2G.Score.noAux",
        y_Aux_col="ENCODE-rE2G.Score.Aux",
        rasterize=True,
        save_path=save_path
        )
    for ax in axes:
...
def plot_log2fc_rE2G_score_scatter(df, output_dir, filename=None):
    # plots change in rE2G score scatter (like Extended Data Fig. 1j)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:
        save_path = None
    fig, ax = plot_rasterized_scatter(
        df,
        x_col='DistanceToTSS.Kb',
        y_col='ENCODE-rE2G.Score.log2FC.(Aux/noAux)',
        rasterize=True,
        save_path=save_path
        )
    plt.close(fig)
...
def plot_retained_hic_boxplot(df, output_dir, filename=None):
    # Plot boxplot of log2FC in hi-c contact for retained vs lost rE2G enhancers (like Extended Data Figure 1e)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:
        save_path = None
...
def plot_retained_dist_boxplot(df, output_dir, filename=None):
    # Plot boxplot of distance to TSS for retained vs lost rE2G enhancers (like Extended Data Figure 1f)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:
        save_path = None
...
def plot_cohesin_distance_boxplot(df_noAux, df_Aux, output_dir, filename=None):
    # Plot boxplot of distance to TSS for rE2G predictions in cohesin present and absent conditions (like Extended Data Figure 1h)
    if filename:
        save_path = os.path.join(output_dir, filename + ".pdf")
    else:
        save_path = None
...
def plot_density_correlation(df, x_col, y_col, xlim=None, ylim=None, save_path=None):
    ...
    print(f"log2FC HiC vs log2FC rE2G Pearson R = {r:.2f}, p = {pval:.2e}")
    # Save plot
    if save_path:
        plt.savefig(save_path, format="pdf", bbox_inches="tight")
        plt.close(fig)
...
def generate_rE2G_gene_df(df_filtered_hic, rE2G_threshold=0.041, hic_threshold = (-0.5), distance_threshold=50):
    # Get unique genes
    genes = df_filtered_hic['TargetGene'].unique()
    df_gene_rE2G = pd.DataFrame({'TargetGene': genes})
    ...
    for gene in genes:
        ...
        enhancers_FC_rE2G = enhancers_FC.loc[enhancers_FC['ENCODE-rE2G.Score.noAux'] >= rE2G_threshold]
        enhancers_distal = enhancers.loc[enhancers['DistanceToTSS.Kb'] >= distance_threshold]
        enhancers_distal_rE2G = enhancers_distal.loc[enhancers_distal['ENCODE-rE2G.Score.noAux'] >= rE2G_threshold]
        total = len(enhancers)
        df_gene_rE2G.loc[df_gene_rE2G['TargetGene'] == gene, [
            'n_enhancers_distal', 'n_enhancers_proximal', 'n_enhancers_hi_HiC', 'n_enhancers_low_HiC',
            'Fraction_enhancers_distal', 'Fraction_enhancers_distal_rE2G',
            'Fraction_enhancers_hic', 'Fraction_enhancers_hic_rE2G',
            'n_enhancers_distal_rE2G', 'n_enhancers_hic_rE2G'
        ]] = [
            ...
            len(enhancers_distal_rE2G) / total,
            len(enhancers_FC) / total,
            len(enhancers_FC_rE2G) / total,
            len(enhancers_distal_rE2G),
            len(enhancers_FC_rE2G)
        ]
    # CLean df
    df_gene_rE2G.dropna(inplace=True)
    return df_gene_rE2G



def subset_group_from_rE2G_gene_df(df_gene_rE2G, df_filtered_group):
    genes = df_filtered_group['TargetGene'].unique()
    return df_gene_rE2G[df_gene_rE2G['TargetGene'].isin(genes)]
    


def calc_gene_enhancer_stats(df_gene_rE2G_group, group_name,
                             column,
                             column_rE2G, label):
    # Compute stats
    total_genes = len(df_gene_rE2G_group)
    distal_count = df_gene_rE2G_group[df_gene_rE2G_group[column] >= 1].shape[0]
    strong_distal_count = df_gene_rE2G_group[df_gene_rE2G_group[column_rE2G] >= 1].shape[0]
    # Print stats
...
def plot_fraction_cdf_and_stats(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr, fraction_col,
                                 labels=None,
                                 colors=None,
                                 threshold=0.5,
                                 save_path=None):
    ...
    # Plot
    fig, ax = plt.subplots(figsize=(5,5))
    sns.ecdfplot(df_gene_rE2G_non[fraction_col], color=colors[0], label=labels[0], lw=4)
    sns.ecdfplot(df_gene_rE2G_mid[fraction_col], color=colors[1], label=labels[1], lw=4)
    sns.ecdfplot(df_gene_rE2G_dr[fraction_col], color=colors[2], label=labels[2], lw=4)
    # Set axes
    ...
    non_total, non_below, non_above = perc_below_above(df_gene_rE2G_non, fraction_col, threshold)
    dr_total, dr_below, dr_above = perc_below_above(df_gene_rE2G_dr, fraction_col, threshold)
    # Print stats
...
def plot_enhancer_count_distribution(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr, column,
    ...
    totals = []
    fractions = []
    for df in [df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr]:
...
def subset_df_tpm(df_gene_rE2G_group, df_tpm, column, label=None):
    # Get genes
    genes = df_gene_rE2G_group['TargetGene'].unique()
    # Subset df
...
def plot_hic_vs_distance(df, enhancers, colors=None, save_path=None):
    ...
    print(f"Number of rE2G enhancers included: {len(df)}")
...
def main(args):
    ...
    save_path = os.path.join(output_dir, "rE2G_dist_hic_rolling" + ".pdf")
    fig.savefig(save_path, format="pdf", bbox_inches="tight")
    fig, ax = plot_rE2G_score_rolling(df_sorted, output_dir)
    save_path = os.path.join(output_dir, "rE2G_dist_rE2G_rolling" + ".pdf")
    fig.savefig(save_path, format="pdf", bbox_inches="tight")
    plot_log2fc_hic_contact_rolling(df_sorted, output_dir, filename="rE2G_dist_log2_hic_rolling")
    plot_log2fc_rE2G_score_rolling(df_sorted, output_dir, filename="rE2G_dist_log2_rE2G_rolling")
    plot_log2fc_percent_retained_rolling(df_sorted, output_dir, filename="rE2G_dist_log2_retained_rolling")

    #Plot supplemental figures using rE2G E-P pairs (Extended Data Fig. 1)
        # scatters
    plot_hic_contact_scatter(df_sorted, output_dir, filename="rE2G_dist_hic_scatter")
    fig, axes = plot_rE2G_score_scatter(df_sorted, output_dir)
    save_path = os.path.join(output_dir, "rE2G_dist_rE2G_scatter" + ".pdf")
    fig.savefig(save_path, format="pdf", bbox_inches="tight")
    plot_log2fc_hic_contact_scatter(df_sorted, output_dir, filename="rE2G_dist_log2_hic_scatter")
    plot_log2fc_rE2G_score_scatter(df_sorted, output_dir, filename="rE2G_dist_log2_rE2G_scatter")
        # histogram
    fig, ax = plot_distance_histogram(df_sorted, output_dir)
    save_path = os.path.join(output_dir, "rE2G_dist_histogram" + ".pdf")
    fig.savefig(save_path, format="pdf", bbox_inches="tight")
        # CDFs
    plot_rE2G_enhancers_cdf(
    ...
    color_present='#6E7CA0',
    color_absent='#8F3237',
    save_path_enhancers= os.path.join(output_dir, "rE2G_n_enhancer_CDF.pdf"), 
    save_path_distance= os.path.join(output_dir, "rE2G_distance_enhancers_CDF.pdf")
    )

    # Plot rE2G boxplots
    plot_retained_hic_boxplot(df_filtered, output_dir, filename="rE2G_retained_hic_boxplot")
    plot_retained_dist_boxplot(df_filtered, output_dir, filename="rE2G_retained_dist_boxplot")
    ...
    plot_density_correlation(df_filtered, "ENCODE-rE2G.Score.log2FC.(Aux/noAux)", 'normalized.HiC.5Kb.log2FC.(Aux/noAux)',
                         xlim=(-10,6), ylim=(-10,6), 
                         save_path=os.path.join(output_dir, "rE2G_log2_rE2G_HiC_density.pdf")
                         )
    
    # Startify rE2G-predictions based on expression category and set these thresholds
    ...
    plot_cdf_comparison(df_filtered_non, df_filtered_mid, df_filtered_dr, "DistanceToTSS.Kb", 
                        ...
                        save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_distance_CDF.pdf")
                        )
    ...
    plot_cdf_comparison(df_filtered_non, df_filtered_mid, df_filtered_dr, "%Change.normalized.HiC.5Kb", 
                        ...
                        save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_changeHiC_CDF.pdf")
                        )
    
    #G enerate per gene summary table of rE2G predictions defining the threshold for distal 
    # and contact sensitive enhancers, and the rE2G score that constitutes 15% effect sizes 
    # from the correlation analysis in "_06_CRUDO_Enhancer_ElementLevel_Visualization.py"   
    df_gene_rE2G = generate_rE2G_gene_df(df_filtered_hic, rE2G_threshold=0.75, hic_threshold = (-0.5), distance_threshold=50)

    # Split into expression groups
    df_gene_rE2G_non = subset_group_from_rE2G_gene_df(df_gene_rE2G, df_filtered_non)
    df_gene_rE2G_mid = subset_group_from_rE2G_gene_df(df_gene_rE2G, df_filtered_mid)
    df_gene_rE2G_dr  = subset_group_from_rE2G_gene_df(df_gene_rE2G, df_filtered_dr)
    ...
    groups = [
        (df_gene_rE2G_non, "Non"),
        (df_gene_rE2G_mid, "Mid"),
        (df_gene_rE2G_dr, "DR")
        ]
    # Define enhancer types: (total_col, strong_col, label)
    enhancer_types = [
            ("n_enhancers_distal", "n_enhancers_distal_rE2G", "distal"),
            ("n_enhancers_hi_HiC", "n_enhancers_hic_rE2G", "contact sensitive")
                ]
    # Loop over both
    ...
    plot_fraction_cdf_and_stats(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr, "Fraction_enhancers_distal",
                                 threshold=0.5,
                                save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_FractionDistal_CDF.pdf")
                                )
    # Compare fraction of contact sensitive enhancers (like Fig. 6d)
    plot_fraction_cdf_and_stats(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr, "Fraction_enhancers_hic",
                                 threshold=0.5,
                                 save_path = os.path.join(output_dir, "rE2G_ExpressionGroup_FractionHiC_CDF.pdf")
                                 )
    ...
    plot_enhancer_count_distribution(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr,
                                 column='n_enhancers_distal_rE2G',
                                 save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_nDistal_bar.pdf")
                                 )
    # For strong contact sensitive enhancers (like Figure 6e)
    plot_enhancer_count_distribution(df_gene_rE2G_non, df_gene_rE2G_mid, df_gene_rE2G_dr,
                                 column="n_enhancers_hic_rE2G",
                                 save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_nHiC_bar.pdf")
                                 )
    
    # Subset TPM df based on gene expression groups to compare TPM distribution across expression groups
    df_tpm_non = subset_df_tpm(df_gene_rE2G_non, df_TPM, "RAD21_NT_avg", "non-DR")
    df_tpm_mid = subset_df_tpm(df_gene_rE2G_mid, df_TPM,  "RAD21_NT_avg","mid-DR")
    df_tpm_dr  = subset_df_tpm(df_gene_rE2G_dr,  df_TPM, "RAD21_NT_avg", "DR")

    # Plot Boxplot TPM comparison for expression groups
    plot_tpm_boxplot(df_tpm_non, 
                    df_tpm_mid, 
                    df_tpm_dr, "RAD21_NT_avg", 
                    save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_TPM_boxplot.pdf")
                    )
    ...
    plot_kde_by_tpm_bin(df_tpm_dr, df_filtered_dr, 
                    ...
                    save_path=os.path.join(output_dir, "rE2G_ExpressionGroup_DR_TPM_density.pdf")
                    )
    ...
    plot_hic_vs_distance(df_filtered_hic, df_enhancers,
                     save_path = os.path.join(output_dir, "rE2G_changeHiC_distance_density.pdf")
                     )
